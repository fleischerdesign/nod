//! `GenerateIsoUseCase`: build bootable installer ISO or image (ADR-021).
...
/// Use case that generates bootable installer ISOs or images.
pub struct GenerateIsoUseCase {
    ctx: Arc<AppContext>,
}

impl GenerateIsoUseCase {
    ...
    pub async fn execute(
        ...
        provisioner.build_iso(target, flake_path, options).await
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn generate_iso_executes_build() {
        let mut prov_mock = MockFakeProvisionerPort::new();
        prov_mock.expect_build_iso().returning(|host, _, _| {
            Ok(IsoReport {
        ...
        let use_case = GenerateIsoUseCase::new(ctx);
        let host = HostEntity::new("installer", "127.0.0.1", true);
        ...
        let report = use_case
            ...
            .await
            .unwrap();
        assert_eq!(report.host_name, "installer");
        assert!(report.ok);
    }
}
