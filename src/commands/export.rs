//! `nod export` command: export fleet inventory to Ansible, Prometheus or JSON (ADR-020).

use std::path::Path;
use std::sync::Arc;

use crate::application::context::AppContext;
use crate::application::use_cases::export_inventory::ExportInventoryUseCase;
use crate::domain::errors::NodError;
use crate::domain::topology::ExportFormat;
...
pub async fn execute(
    ...
    let use_case = ExportInventoryUseCase::new(Arc::new(ctx));
    let exported = use_case.execute(flake_path, format, verbose).await?;
    println!("{exported}");
    Ok(())
}
...
mod tests {
    ...
    #[tokio::test]
    async fn export_command_executes_successfully() {
        ...
        let res = execute(ctx, Path::new("."), ExportFormat::Ansible, false).await;
        assert!(res.is_ok());
    }
}
